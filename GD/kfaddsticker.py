...
def img2sticker_kf(img_orig, img_sticker, detector_hog, landmark_predictor):
    ...
    h_ratio,w_ratio = img_rgb.shape[0] / 360, img_rgb.shape[1] / 640
    # detector
    ...
    for dlib_rect, landmark in zip(dlib_rects, list_landmarks):
        x = int(landmark[30][0] * w_ratio+0.5) # nose
        y = int(landmark[30][1] * h_ratio + 0.5)
        w = int(dlib_rect.width() * w_ratio +0.5)
        h = int(dlib_rect.height() * h_ratio+ 0.5)
    ...
    if refined_y < 0:
        ...
        refined_y = 0
    elif refined_y + img_sticker.shape[0] >= img_orig.shape[0]:
        img_sticker = img_sticker[:-(img_sticker.shape[0] + refined_y - img_orig.shape[0])]
